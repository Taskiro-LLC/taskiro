var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var _this = this;
var LoginForm = /** @class */ (function () {
    function LoginForm() {
        this.emailInput = document.querySelector('#email');
        this.passwordInput = document.querySelector('#password');
    }
    LoginForm.prototype.validateFields = function () {
        var isValid = true;
        if (!this.emailInput.value) {
            this.emailInput.classList.add('invalid');
            isValid = false;
        }
        if (!this.passwordInput.value) {
            this.passwordInput.classList.add('invalid');
            isValid = false;
        }
        return isValid;
    };
    LoginForm.prototype.clearValidation = function () {
        this.emailInput.classList.remove('invalid');
        this.passwordInput.classList.remove('invalid');
    };
    LoginForm.prototype.getUser = function () {
        if (!this.validateFields()) {
            var invalidFields = document.querySelectorAll('.invalid');
            invalidFields.forEach(function (field) {
                field.placeholder = 'This is a required field';
            });
            return null;
        }
        var email = this.emailInput.value;
        var password = this.passwordInput.value;
        return {
            email: email,
            password: password,
        };
    };
    return LoginForm;
}());
var loginUser = function () { return __awaiter(_this, void 0, void 0, function () {
    var form, user, users, matchedUser, mismatched;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                form = new LoginForm();
                form.clearValidation();
                user = form.getUser();
                if (!user) return [3 /*break*/, 3];
                return [4 /*yield*/, fetch('http://localhost:3000/users')];
            case 1: return [4 /*yield*/, (_a.sent()).json()];
            case 2:
                users = (_a.sent());
                matchedUser = users.find(function (u) { return u.email === user.email && u.password === user.password; });
                if (matchedUser) {
                    if (matchedUser.type === 'admin') {
                        console.log(matchedUser);
                        console.log('Logged in as:', matchedUser.email);
                        window.location.href = '../admin-dashboard/admin-dashboard.html';
                        history.replaceState(null, '', '../admin-dashboard/admin-dashboard.html');
                    }
                    else {
                        window.location.href = '../user-dashboard/user-dashboard.html';
                        history.replaceState(null, '', '../user-dashboard/user-dashboard.html');
                    }
                }
                else {
                    mismatched = document.querySelector('.mismatched');
                    mismatched.style.display = 'block';
                }
                _a.label = 3;
            case 3: return [2 /*return*/];
        }
    });
}); };
var signInBtn = document.querySelector('.login-btn');
signInBtn.addEventListener('click', loginUser);
